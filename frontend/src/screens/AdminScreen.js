...
export function Bookings() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const data = await (
          await axios.get("http://localhost:3004/bookings")
        ).data;
        setBookings(data);
        setLoading(false);
      } catch (error) {
...
export function Location() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const data = await (await PackagesApi.getAllPackages()).data;
        setLocation(data);
        setLoading(false);
      } catch (error) {
        console.log(error);
            ...
            location.map((locations) => {
              return (
                <tr key={locations._id}>
                  <td data-label="Name : ">{locations.name}</td>
                  <td data-label="Reviews : ">{locations.reviews}</td>
                  <td data-label="Phonenumber : ">{locations.phonenumber}</td>
                </tr>
              );
            })}
        </tbody>
    ...
    <div className="row">
      <div className="col-md-5 admin-add-new">
        ...
        <div style={{ width: "30%" }}>
          ...
          <input
            type="text"
            placeholder="location rent per day"
            value={lrentperday}
            onChange={(e) => {
